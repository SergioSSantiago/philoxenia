...
| `location`, `locationLat`, `locationLng` | Human-readable place + required map pin (WGS84). Create-listing copy: friends see the pin on Home to **Book & pay** — not a public directory. Empty picker: **Pin this place on the map for guests to Book & pay.** Map search fail: **Could not find that place**. Out-of-range pin: **Pin a valid place on the map**. Listing detail **Open on OpenStreetMap** is an external OSM tab |
