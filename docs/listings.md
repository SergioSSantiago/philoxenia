...
| `connectorRewardPercent` | 0–100; **connector’s share** when a friend shares a place invite. Create field **Connector % of stay** (not “Connector reward (%)”). Helper: 3–10% typical when someone **Book & pay through that place invite** (not “when they Book & pay in STRK or DAI”); Philoxenia takes 10% of that reward only; **Direct Book & pay** stays 0% protocol ([connectors.md](./connectors.md)) |
| `photos` | 1–8 images (JPEG/PNG/WebP/HEIC); browser compresses to JPEG data URLs (~1600px). Uploader copy **iPhone HEIC included** (not “on Safari”) |
...
`GET /my-listings` powers `/my-listings` and Home **My places**: both subtitles **Priced in DAI. Book & pay STRK or DAI** (not “Private places priced in DAI” / “Guests Book & pay”), and connector %. Empty states tell hosts they **haven’t published a place yet** — **share a place invite and earn** (not “introduce guests”). **List your place** goes to `/listings/new` (submit **Publish this place**; validation: **Open at least one night to Book & pay**; default DAI: **Enter a valid DAI list price / night**; default DAI price is used when you open those nights). Home **Places from friends to Book & pay** (not “Places from my friends”) empty: no places from friends yet. **Place invites to Book & pay** (not “Shared with me”) subtitle: **Last place invite wins connector % of stay** (not “connector attribution”); empty: **No place invites yet**. Home **My places** / `/my-listings` subtitle: **share a place invite and earn**. Home **My stays**: recent Book & pay stays; empty **wait after a place invite** (not “wait for a guest”). Home **Friends to Book & pay** (not “My friends”). Home loaders: **Loading places to Book & pay…**.
