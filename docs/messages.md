...
| `place_invite` | Friend shares a place they can introduce: card in the thread with photo, price, host, and **Open to Book & pay** → `/invite/{token}`. Creates a listing share; if the sharer is not the host they are the connector. **Attribution is saved when the place invite is sent** (last-touch for that guest + listing), so the connector earns on settle even if the guest never opened an external link. Header CTA **Share place invite** / **Close place invite**. Picker lists friends’ places + your own (not the peer’s own places). Host share → no connector reward. Bell: **Place invite in Messages**. Thread preview uses the plaintext body (`Place invite: …`). |
...
| GET | `/messages/:friendId` | Yes | Conversation (ciphertext for sealed texts; place invite cards include `placeInvite`) |
| POST | `/messages/:friendId` | Yes | Send `{ body }` — prefer sealed `phx1.…` (max 8000) |
| POST | `/messages/:friendId/place-invite` | Yes | Send `{ listingId }` — create place invite card + attribute recipient |
