...
3. Live network totals under the brand (`LandingNetworkStats`): users, countries (normalized from address country, e.g. Suiza→switzerland), listings open, **paid nights**, **DAI paid**, **STRK paid** — polled from `GET /stats/network` every **20s**. Labels say paid because **Book & pay** settles in the same tx.
