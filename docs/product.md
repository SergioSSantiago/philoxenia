...
- **Ready X** (page title, not “Profile”): display name (label **Display name (friends add you by Ready X wallet)**, not “Name shown to others”; CTA **Save display name**; busy **Saving display name…**; done **Display name saved**, not “Saved!”), public STRK/DAI balances, shield/unshield **STRK or DAI**, **AVNU STRK ↔ DAI swap**. Subtitle: friends add you by **Ready X wallet**. Display name is not searchable.
