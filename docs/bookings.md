...
`/bookings/new` calendar copy: tap nights one by one; they need not be consecutive; then **Book & pay** in STRK or DAI. Empty selection: **Tap nights to Book & pay.** No open nights: ask the host to open nights you can **Book & pay**. Unauthorized place: **This place isn’t available to Book & pay.** Quote labels **Book & pay with** (STRK or DAI) and **Book & pay now**. Primary button is **Book & pay {amount} STRK/DAI**; busy: **Book & pay…** or **Private Book & pay…**; idle Private suffix is **· Private**. Guest calendar: past nights cannot be added to Book & pay. Own place: **You cannot Book & pay your own place.** **Back to place.** Missing query: **Missing place.** If the JWT is live but the wallet is not, the page asks to **Connect Ready X** (Chrome or iPhone) to **Book & pay** — Public and Private both need a live session; Private also needs wallet API ≥ 0.10. Reconnect failures and Private Book & pay errors name Ready X (not a generic Ready extension). Wallet dismiss: **Book & pay cancelled in Ready X.** Missing hash: **Book & pay did not return a transaction hash**. Recording copy: do not tap **Book & pay** again — open My bookings. There is no silent fallback to Public Book & pay.

After a successful on-chain pay, the client stores a pending row in `localStorage` (`philoxenia_pending_paid_booking`) and retries `POST /bookings/confirm` (Ready X sometimes throws after the tx already landed). **Recording stay…** only appears when that pending row (or an in-flight Book & pay) exists — a normal Book & pay visit is not locked. Intro and recording errors: **Do not Book & pay again** — open My bookings (also if Ready X charged you). Exhausted confirm retries: **Could not record Book & pay stay**. Quote and stay detail label **Book & pay privacy** (Public / Private). Reopening the page resumes confirm until it succeeds; `/bookings` loading is **Loading stays…**; empty: **No Book & pay stays yet**; stay detail **Loading stay…**; pay page **Loading place to Book & pay…**. Subtitle: **Book & pay is STRK or DAI**; cancel only frees nights.
...
| GET | `/bookings` | List — empty **No Book & pay stays yet**; Home empty matches. Cards show **You host** / **You stay**. Home **My bookings** subtitle: recent Book & pay stays. If nights are gapped, the card lists those dates (or “not consecutive”) instead of a contiguous check-in–out range |
