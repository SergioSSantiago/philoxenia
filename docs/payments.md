...
| Surface | Mode | Notes |
|---------|------|--------|
| `/home` | `compact` | Public STRK **and DAI** hints “shield for Private Book & pay”. Crash fallback: **Balances unavailable right now. Ready X is still connected — open Shield & swap on Ready X.** (not “try Shield & swap”). Compact hint **shield STRK or DAI on Ready X** (not “on Profile”). |
