...
| `/profile` | full | Same public rows + `Strk20PrivacyPanel` heading **Shield for Private Book & pay** (shield / unshield **STRK or DAI**). Helper **Shield and unshield amounts are public on-chain** (not “Deposit/withdraw … onchain”). Busy shield: **Approve {asset} shield (public amount)** (not “deposit”). Disconnect heading **Ready X in this browser** (not “Session”). Notices: **Ready X session needed** (not a generic Ready session); shield/unshield fail **Could not shield or unshield STRK or DAI**. Incapable wallet: **This Ready X wallet does not expose STRK20 yet** |
