...
Listing cards (`ListingCard`) show `{n}% connector of stay` (not “connector reward”) next to the DAI price when the host set a reward. Helper **Book & pay: STRK or DAI** (not “Guest Book & pay”). Badge **Publishes this place** (not “Host”). Empty photo: **No place photo**.
...
2. If already friends with host → the place opens (**Opening place to Book & pay…**). First paint while resolving the token: **Opening place invite to Book & pay…**. Pending friendship: open the **place** so you can Book & pay. Invite eyebrow: **Place invite**. Invalid/expired token (API + page): **This place invite isn’t available to Book & pay.** Invite body: **Place hosted by** (not “Private place hosted by”). Host badge **Publishes this place** (not “Host”). Already friends when requesting: **You’re already friends — Book & pay their places from Friends.** Connector empty: **None — they published this place invite** (not “None — host place invite”). Host share: **This place invite has no connector reward.** (not “This host place invite”). Connector share: **If you Book & pay through this place invite**, the reward goes to their Ready X wallet.
3. If not → **Request friendship to Book & pay with {name}** (not “Request friendship with”); after accept, the place unlocks. Friend-request fail: **Could not send friend request to Book & pay** (not “Could not send the friend request”). Busy: **Sending friend request to Book & pay…** (not “Sending…”). While pending, `/invite/[token]` polls every **3s** and redirects when `canViewListing` becomes true (copy: open the place so you can **Book & pay**).
4. Display name is cosmetic; rewards always go to the connector **Ready X wallet**. Unsigned guests tap **Connect Ready X to Book & pay** on `/invite/[token]`. Connector badge: **Earns as connector** (not “Connector (Ready X payout)”). Empty heading **Earns as connector** (not “Connector”). Pending **Friend request sent** (not “Friendship request sent”). Invite body: friends with **who publishes this place** to **Book & pay (STRK or DAI)** (not “friends with the host”); if they **Book & pay** through the place invite, the reward goes to the connector’s Ready X wallet.
