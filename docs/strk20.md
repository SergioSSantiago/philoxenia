...
1. **Wallet API** — shield / unshield / balances for STRK and DAI (`WalletAccountV6`, wallet API ≥ 0.10). **Ideal:** Ready X in-app browser. Firefox / desktop legacy Ready usually lacks ≥ 0.10 → Private unavailable. Profile, swap, chat, and Private Book & pay errors say **Ready X** (Chrome or iPhone). When Profile cannot see ≥ 0.10, the shield panel lists Chrome + Ready X (not Firefox) **or** iPhone in-app browser, then Smart Wallet + Private, then Reconnect Ready X.
