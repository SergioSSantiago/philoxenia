...
function notCapableReason(versions: string[], hadWallet: boolean): string {
  ...
  if (versions.length === 0) {
    return fx
      ...
      : "Ready X is connected for login, but Private Book & pay needs Chrome with the Ready X wallet extension, or Philoxenia in the Ready X browser (wallet API ≥ 0.10). Unlock Ready X, then tap Reconnect Ready X. If it still fails, Refresh Philoxenia.";
  }

  return `Ready X wallet API is ${versions.join(", ")} (need ≥ 0.10 for Private Book & pay). Update Ready X${fx ? " — Firefox has no Ready X; Chrome with the Ready X wallet extension is recommended, or open Philoxenia in the Ready X browser" : ""}, enable Private, and reconnect.`;
}
